...
class Trainer(object):
    ...
    def eval(self, epoch):
        ...
        self.writer.add_scalar('VAL/Bleu1', val_res['Bleu_1'], epoch+1)
        self.writer.add_scalar('VAL/Bleu2', val_res['Bleu_2'], epoch+1)
        self.writer.add_scalar('VAL/Bleu3', val_res['Bleu_3'], epoch+1)
        self.writer.add_scalar('VAL/Bleu4', val_res['Bleu_4'], epoch+1)
        self.writer.add_scalar('VAL/Meteor', val_res['METEOR'], epoch+1)
        self.writer.add_scalar('VAL/Rouge', val_res['ROUGE_L'], epoch+1)
        self.writer.add_scalar('VAL/Cider', val_res['CIDEr'], epoch+1)
        ...
        self.writer.add_scalar('TEST/Bleu1', test_res['Bleu_1'], epoch+1)
        self.writer.add_scalar('TEST/Bleu2', val_res['Bleu_2'], epoch+1)
        self.writer.add_scalar('TEST/Bleu3', val_res['Bleu_3'], epoch+1)
        self.writer.add_scalar('TEST/Bleu4', val_res['Bleu_4'], epoch+1)
        self.writer.add_scalar('TEST/Meteor', test_res['METEOR'], epoch+1)
        self.writer.add_scalar('TEST/Rouge', test_res['ROUGE_L'], epoch+1)
        self.writer.add_scalar('TEST/Cider', test_res['CIDEr'], epoch+1)
        self.writer.flush()
        self.test_total_score = test_res['Bleu_1'] + test_res['Bleu_2'] + test_res['Bleu_3'] + test_res['Bleu_4'] + test_res['METEOR'] + test_res['ROUGE_L'] + test_res['CIDEr']

        val = 0
        for score_type, weight in zip(cfg.SCORER.TYPES, cfg.SCORER.WEIGHTS):
            if score_type == 'Bleu':
            	val -= 2*val_res['Bleu_1'] + 2*val_res['Bleu_2'] + 1*val_res['Bleu_3'] + 1*val_res['Bleu_4']
            	# val -= val_res['Bleu_4'] * weight           # TODO 
            else:
    ...
    def train(self):
        ...
        for epoch in  range(cfg.SOLVER.MAX_EPOCH):
            ...
            val = self.eval(epoch)
            # score_data = pd.DataFrame({'epoch': epoch, 'total_score': self.test_total_score})
