...
class AttBasicModel(BasicModel):
    ...
    def decode(self, **kwargs):
        ...
        unfinished = wt.eq(wt)
        trigrams = []                 # TODO
        kwargs = self.make_kwargs(wt, gv_feat, att_feats, att_mask, p_att_feats, state)   # TODO
        logprobs_t, state = self.get_logprobs_state(**kwargs) # TODO
        for t in range(cfg.MODEL.SEQ_LEN):
            # kwargs = self.make_kwargs(wt, gv_feat, att_feats, att_mask, p_att_feats, state)
            # logprobs_t, state = self.get_logprobs_state(**kwargs) # TODO
            if greedy_decode:
            ...
            if t>=2 and greedy_decode:
                ...
                prev_two_batch = sents[:, t-1: t+1]
                mask = torch.zeros(logprobs_t.size(), requires_grad=False).cuda()
                for i in range(batch_size):
                    prev_two = (prev_two_batch[i][0].item(), prev_two_batch[i][1].item())
                    if prev_two in trigrams[i]:
                        for j in trigrams[i][prev_two]:
                            mask[i,j] += 1
                alpha = 2.0
                # logprobs_t = logprobs_t + (mask * -0.693 * alpha)
                logprobs_t = logprobs_t + torch.exp(-mask)-1
