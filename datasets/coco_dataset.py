...
class CocoDataset(data.Dataset):
    def __init__(
        ...
        if input_seq is not None and target_seq is not None:
            ...
            # print(self.input_seq[self.image_ids[0]])
            self.seq_len = len(self.input_seq[self.image_ids[0]][:])
        else:
            self.seq_len = -1
            self.input_seq = None
    ...
    def __getitem__(self, index):
        ...
        if self.att_feats_folder is not None:
            # self.att_feats_folder = './data/mimic_att'
            # att_feats_frontal = np.load(os.path.join(self.att_feats_folder,  str(image_id) + '-frontal.npz'))['feat']
            # att_feats_lateral = np.load(os.path.join(self.att_feats_folder,  str(image_id) + '-lateral.npz'))['feat']
            att_feats_frontal = np.load(os.path.join(self.att_feats_folder,  'CXR'+str(image_id) + 'frontal.npz'))['feat']
            att_feats_lateral = np.load(os.path.join(self.att_feats_folder,  'CXR'+str(image_id) + 'lateral.npz'))['feat']
            att_feats_frontal = att_feats_frontal.reshape(-1, att_feats_frontal.shape[-1])
            att_feats_lateral = att_feats_lateral.reshape(-1, att_feats_lateral.shape[-1])
            att_feats = np.concatenate((att_feats_frontal, att_feats_lateral), 1)
        ...
        for i, ix in enumerate(ixs):
            input_seq[sid + i] = self.input_seq[image_id][:]       # TODO
            target_seq[sid + i] = self.target_seq[image_id][:]
